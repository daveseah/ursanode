...
Lerna works with both `npm` and `yarn`. In URSANODE we are using `npm`. 
...
In URSANODE, our monorepo includes the additional tooling:
...
Assume you want to add a new package named `mypackage` using the `@ursanode` scope. Make sure you are in the monorepo root, then issue the following commands:

```
lerna create @ursanode/mypackage -y     # create npm package skeleton in gs_packages
```

You should look at the `package.json` and update it as necessary. Make sure you set it to private, since we're not publishing URSANODE as a public resource available via npm.
...
lerna add @ursanode/mypackage --scope=@ursanode/anotherapp1
lerna add @ursanode/mypackage --scope=@ursanode/anotherapp2
...
```
npm ERR! 404  '@ursanode/globals@^0.0.0' is not in the npm registry.
